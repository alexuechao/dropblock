...
class DropBlock(mx.operator.CustomOp):
    def __init__(self, drop_prob, block_size,batch_size,channel):
        super(DropBlock, self).__init__()
        self.drop_prob = drop_prob
        self.block_size = block_size
        self.batch_size = batch_size
        self.channel = channel
        self.block_mask = nd.ones((self.batch_size, self.channel, 7, 7), ctx=mx.gpu(0))
    ...
    def _compute_block_mask(self, mask):
        weight_mat = nd.ones(self.channel*self.block_size*self.block_size)
        weight_mat = weight_mat.reshape(
            [self.channel, 1, self.block_size, self.block_size])
        block_mask = nd.Convolution(data=mask, no_bias=True, weight=weight_mat, num_filter=self.channel, kernel=(
...
class DropBlockProp(mx.operator.CustomOpProp):
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return DropBlock(self._drop_prob, self._block_size,self.batch_size,self.channel)